...
export const CopyButton = ({
  ...
  className?: string;
  copyButtonVariant?: Parameters<typeof Button>[0]["variant"];
}) => {
  ...
  React.useEffect(() => {
    if (justCopied) {
      ...
      copyAnimationTimerRef.current = setTimeout(
  ...
  return (
    <div className={cn("w-fit h-fit", className, "relative")}>
      <Button
        className={cn(animationClassName, "absolute w-full h-full", {
      ...
      <Button
        className={cn(animationClassName, "w-full h-full", {
