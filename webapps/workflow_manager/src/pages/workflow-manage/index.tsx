  ...
  return (
    <PageContext.Provider value={pageStore}>
      <Dialog open={!!pageStore.dialogContext.current}>
        ...
        <MobileManager workflowId={workflowIdN} />
      </Dialog>
    </PageContext.Provider>
  );
});

const MobileManager = ({ workflowId }: { workflowId: number }) => {
  return <div className="md:hidden"></div>;
};

const DesktopManager = ({ workflowId }: { workflowId: number }) => (
  <div className="hidden md:block">
    <div className="flex flex-col gap-4">
      <desktop.WorkflowTitle workflowId={workflowId} />
      <div className="w-4/5 mx-auto">
        <desktop.StateCollapserList workflowId={workflowId} />
      </div>
    </div>
    <desktop.PageDialogContent />
  </div>
);

module desktop {
  export const WorkflowTitle = observer(
  ...
  export const StateCollapserList = observer(
    ({ workflowId }: { workflowId: number }) => {
      const workflowStore = useWorkflowStore();
      const workflow = workflowStore.workflows.get(workflowId);
  ...
  export const PageDialogContent = observer(() => {
    const pageStore = React.useContext(PageContext);
    ...
    return (
      <>
        ...
        >
          ...
          {newEntityCtx ? (
            <NewEntityContext newEntityCtx={newEntityCtx} />
          ) : null}
        </DialogContent>
      </>
    );
  ...
  const StateContext = observer(
    ({ ctx }: { ctx: Parameters<typeof AttributesForm>[0] }) => {
      ...
      const stateName = workflowStore.workflowStates.get(
        ctx.baseEntityId,
      )?.name;
