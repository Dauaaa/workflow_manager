...
public class WorkflowController {
    ...
    public WorkflowController(WorkflowRepository workflowRepository, WorkflowStateRepository workflowStateRepository) {
    ...
    public Workflow getWorkflow(@PathVariable("workflowId") Integer workflowId) {
        return ErrorUtils.onEmpty404(this.workflowRepository.getByIdAndClientId(workflowId, 1), workflowId);
    }
    ...
    @ResponseBody
    public Workflow setConfig(@PathVariable("workflowId") Integer workflowId, @RequestBody WorkflowConfigDTO config) {
        ...
        if (config.initialStateId != null) {
            initialState = ErrorUtils.onEmpty404(this.workflowStateRepository.getByIdAndClientId(config.initialStateId, 1), config.initialStateId);
        }
